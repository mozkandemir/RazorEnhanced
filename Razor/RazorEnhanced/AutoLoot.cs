...
namespace RazorEnhanced
{
    ...
    public class AutoLoot
    {
        ...
        private static Queue<int> m_IgnoreLootedCorpseList = new Queue<int>();
        private static bool m_lootsuccess = false;
        ...
        private static bool IsIgnoredCorpse(RazorEnhanced.Item corpse)
        {
            if (m_IgnoreLootedCorpseList.Contains(corpse.Serial))
            {
        ...
        internal static void Engine(Dictionary<int, List<AutoLootItem>> autoLootList, int mseconds, Items.Filter filter)
        {
            ...
            foreach (RazorEnhanced.Item corpse in corpi)
            {
                if (!IsIgnoredCorpse(corpse))
                {
                    ...
                    List<int> itemsToGrabSerial = new List<int>();
                        ...
                        foreach (AutoLootItem autoLootItem in matchAll)
                        {
                            if (autoLootItem.Selected)
                            {
                                if (autoLootItem.Color == item.Hue || autoLootItem.Color == -1)
                                {
                                    ...
                                    itemsToGrabSerial.Add(item.Serial);
                                }
                            }
                        }
                    ...
                    foreach (RazorEnhanced.Item item in m_cont.Contains)
                    {   
                        if (itemsToGrabSerial.Exists(check => check == item.Serial))
                        {
                            m_lootsuccess = false;
                        }
                    ...
                    if (m_lootsuccess)
                    {
                        m_IgnoreLootedCorpseList.Enqueue(m_cont.Serial);
                        if (m_IgnoreLootedCorpseList.Count > 200)
                            m_IgnoreLootedCorpseList.Dequeue();
                    }
                }
            }
        }
        ...
        public static void ResetIgnore()
        {
            ...
            m_IgnoreLootedCorpseList.Clear();
